    ...
    try:
        if state.get("raw_input_jd"):
            ...
            job_order = JobOrder(
                ...
                client_name=state.get("client_name_override") or client_name,
                role_name=state.get("role_name_override") or parsed_data.get("role_name") or "Software Engineer",
                required_skills=state.get("required_skills_override") or parsed_data.get("required_skills") or [],
                nice_to_have_skills=state.get("nice_to_have_skills_override") or parsed_data.get("nice_to_have_skills") or [],
                budget_max=state.get("budget_max_override") or parsed_data.get("budget_max") or 120000.0,
                location=state.get("location_override") or parsed_data.get("location") or "Remote",
